...
class ValuePropagation:
    def __init__(self, num_agents, state_dim, action_dim, pi, eta=0.1, lmbda=0.01, gamma=0.99, T_dual=4, adam_lr=5e-4):
        ...
        self.value_optimizer = torch.optim.Adam(self.value_net.parameters(), lr=self.adam_lr)

        self.policy_net = PolicyNet(self.state_dim, self.action_dim)
        self.policy_optimizer = torch.optim.Adam(self.policy_net.parameters(), lr=self.adam_lr)

        self.dual_net = DualFuncNet(self.state_dim, self.num_agents)
        self.dual_optimizer = torch.optim.Adam(self.dual_net.parameters(), lr=self.adam_lr)
    ...
    def calc_dual_obj2(self, idx, transition_dict):
        ...
        delta_add_penalty = self.calc_delta_add_penalty(idx, transition_dict).detach()
        sa_pair = torch.cat([states, actions_list], dim=1)
        rho = self.dual_net(sa_pair).detch()
        return -self.eta * (delta_add_penalty -rho)**2
