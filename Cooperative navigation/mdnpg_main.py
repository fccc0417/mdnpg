...
def run(args, env_name):
    ...
    # Generate an training agent list.
    agents = []
    ...
    for i in range(10):
        with tqdm(total=int(args.num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(args.num_episodes / 10)):
                # Copies of training agents in the last time.
                phis_list = []
                for agent in agents:
                ...
                for idx, (agent, env) in enumerate(zip(agents, envs)):
                    minibatch_v = []  # list of minibatch gradient v^t for each agent
                    states_list = []  # list of state for each agent
                ...
                # Take gradient consensus for y^t.
                ...
                # Take gradient consensus for H^t*y^t.
                consensus_grad_lists = take_grad_consensus(update_grad_lists, pi)
                # Take parameter consensus for \theta^t.
                ...
                y_lists = copy.deepcopy(next_y_lists)  # copies of last y
                prev_v_lists = copy.deepcopy(v_lists)  # copies of last v
