            ...
            for i_episode in range(int(args.num_episodes / 10)):
                ...
                for idx, (agent, env) in enumerate(zip(agents, envs)):
                    ...
                    for b in range(args.minibatch_size):
                        ...
                        single_traj_u = agent.compute_u_k(transition_dict, advantage, prev_u_lists[idx], phis_list[idx],
                                                          args.beta)
                ...
                consensus_next_v_k_lists = take_grad_consensus(next_v_k_lists, pi)
                agents = take_param_consensus(agents, pi)

                for agent, grads in zip(agents, consensus_next_v_k_lists):
