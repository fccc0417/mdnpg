...
def run(args, env_name):
    ...
    fpath2 = os.path.join('records', 'batch_md_npg_gt_logs', str(num_agents) + 'D', 'beta=' + str(args.beta), topology)
    ...
    for i in range(args.num_agents):
        agents.append(MomentumNPG(num_agents=args.num_agents, state_dim=len(sample_obs), action_dim=4, lmbda=args.lmbda,
...
if __name__ == '__main__':
    ...
    topology = "bipartite"  #'dense', bipartite, ring
    num_agents = 3
    ...
    for beta, label in zip(betas, labels):
        ...
        fpath = os.path.join('batch_md_npg_gt_results', env_name, str(num_agents) + 'D',
